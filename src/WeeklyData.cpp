...
void WeeklyData::generateData() {
   ...
   generateLocationsAndTravelTimes();
...
void WeeklyData::writeDailyData(int day) const {
   ...
   // Location positions.
   fid << "x\n";
   fid << get<0>(m_locations.front()) << " ";
   for (int i: m_patients[day])
      fid << get<0>(m_locations[i]) << " ";
   fid << get<0>(m_locations.front()) << "\n";

   fid << "y\n";
   fid << get<1>(m_locations.front()) << " ";
   for (int i: m_patients[day])
      fid << get<1>(m_locations[i]) << " ";
   fid << get<1>(m_locations.front()) << "\n";
...
void WeeklyData::generateLocationsAndTravelTimes() {
   // +1 because of the depot.
   m_locations.resize(m_poolSize+1);
   m_travelTimes.resize(m_locations.size());
   for (auto &i: m_travelTimes)
      i.resize(m_locations.size(), 1e6);

   for (auto &l: m_locations)
      l = m_params.locationGenerator().generatePoint();

   for (size_t i = 0; i < m_locations.size(); ++i) {
      for (size_t j = 0; j < m_locations.size(); ++j) {
         m_travelTimes[i][j] = m_params.locationGenerator().travelTimeMinutes(
            m_locations[i],
            m_locations[j]
         );
      }
